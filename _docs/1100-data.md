...
pushing the pedals with a cadence greater than 60 revs per minute. With other words in one second then I do more than
one rotation in one second. When I additionally process data from a power meter (pedals) then it's IMHO essential to
...
especially when reviewing/comparing different powermeters (based on data recorded with 'BLACKBOX' Garmin bike computers
...
- Timestamp (in milliseconds)
- Containing data package (as plain bytes[])
- Beacon address (as String)
- RSSI (as Number \[Integer\])
- NanoSeconds (as Number \[long\])
- Total Number of visible beacons including _this_ (at the time this beacon was detected for the first time) 

## <span class="material-icons" style="width: 1.042em;">sensors</span> External Sensors
Details about the currently [supported additional external sensors](../3000-sensors/) can be found in the corresponding
section of this manual. This secon here covers only the data fields that will be recorded per sensor.
...
### <i class="fa-solid fa-bolt fa-fw"></i> Powermeter Data
- Timestamp (in milliseconds)
- Current power (in _WATTs_ \[as integer\])
- PowerMeter Index (could be 0: center, 1:left, 2:right)
...
- Timestamp (in milliseconds)
- Front gear position (0 or 1)
- Rear gear position (0-12)
